//static DAY5: &str = include_str!("../data/day5.txt");
use std::{collections::BTreeMap, cmp::min, time::Instant};
...
impl Map {
    ...
    pub fn new2(mapping : &str) -> Result<Self,()> {
        let mut map: BTreeMap<u64,(u64,u64)> = BTreeMap::new();

        let vec : Vec<u64> = mapping.split_whitespace().map(|x| x.parse::<u64>().unwrap()).collect();

        for pair  in  vec.chunks(2){
            map.insert(pair[0], (pair[0], pair[1]));
        }
        Ok(Self {map})
    }

    pub fn get(&self,from : &u64) ->u64 {
        if let Some(key) = self.map.keys().rev().find(|&&key|  key <= *from) {
            let (to, count) = self.map[key];
            let diff = from - key;
    ...
    fn next(&self, from : &u64, to : &u64) -> Option<&u64> {
        self.map.keys().find(|&&k| k > *from).and_then(|k| {
            if k < to {
                return Some(k)
            }
            None
    ...
    fn get_avail(&self,from : &u64) -> (u64, u64) {
        if let Some(key) = self.map.keys().rev().find(|&&key|  key <= *from) {
            let (to, count) = self.map[key];
            let diff = from - key;
    ...
    pub fn combine(&self, other : &Map ) -> Self{
        let mut map :BTreeMap<u64, (u64, u64)>  = BTreeMap::new();

        // for every from replace to from other
        for (from, (to, count)) in self.map.iter() {
            let mut from = *from;
            let mut to = *to;
            let mut count = *count;

            while count >0 {
                let (new_to, avail) = other.get_avail(&to);
                let available = min(count, avail);
    
                if available > 0 { // Mapping exists
                    map.insert(from, (new_to, available));
                    from += available;
                    to += available;
                    count -= available;
                }

                // Fill with zeros until next key
                else if let Some(new_to) =  other.next(&to, &(to+count)) {
                    let zero_count = new_to  - to;
                    
                    map.insert(from, (to, zero_count));
                    from += zero_count;
                    to += zero_count;
                    count -= zero_count;
...
fn main() {
    let seeds =  "432563865 39236501 1476854973 326201032 1004521373 221995697 2457503679 46909145 603710475 11439698 1242281714 12935671 2569215463 456738587 3859706369 129955069 3210146725 618372750 601583464 1413192"
        .split_whitespace().map(|v| v.parse::<u64>().unwrap());

    let seeds_map = Map::new2("432563865 39236501 1476854973 326201032 1004521373 221995697 2457503679 46909145 603710475 11439698 1242281714 12935671 2569215463 456738587 3859706369 129955069 3210146725 618372750 601583464 1413192").unwrap();



    let seed_to_soil = Map::new(
    ...
    let soil_to_fertilizer = Map::new(
        ...
        0 61048150 249083929").unwrap();


    let fertilizer_to_water = Map::new(
        ...
        2414467031 1776448144 81703655").unwrap();


    let water_to_light = Map::new(
        ...
        936681812 1203751669 100352990").unwrap();

    let light_to_temperature = Map::new(
        ...
        316819699 699963220 164194067").unwrap();

    let temperature_to_humidity =  Map::new(
        ...
        3041141586 3009135359 95892752").unwrap();

    let humidity_to_location = Map::new(
        ...
        1727865346 4102106917 192860379").unwrap();


    let soil = seeds.map(|v| seed_to_soil.get(&v));
    let fertilizer = soil.map(|v| soil_to_fertilizer.get(&v));
    let water = fertilizer.map(|v| fertilizer_to_water.get(&v));
    let light = water.map(|v| water_to_light.get(&v));
    let temperature = light.map(|v| light_to_temperature.get(&v));
    let humidity = temperature.map(|v| temperature_to_humidity.get(&v));
    let location = humidity.map(|v| humidity_to_location.get(&v));

    let min = location.min().unwrap();
    ...
    let combined = seeds_map.combine(&seed_to_soil)
                                .combine(&soil_to_fertilizer)
                                .combine(&fertilizer_to_water)
                                .combine(&water_to_light)
                                .combine(&light_to_temperature)
                                .combine(&temperature_to_humidity)
                                .combine(&humidity_to_location).min_to();

    println!("Min Combined: {}", combined);
...
fn part1() -> usize {
    ...
    let sections = Vec::from_iter(DAY5.split("map:"));
